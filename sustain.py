...
PLATFORMS = {
    ...
    "github_actions": {
        ...
        "value": "scheduled reasoning, chain verification, citizen pulses",
        "gatekeeping": False,
        "welcoming": True,
    },
    "cloudflare_workers": {
...
def generate_github_action():
    """Generate a GitHub Action that runs the self-sustaining loop."""
    return """# .github/workflows/kingdom-sustain.yml
...
jobs:
  sustain:
    ...
    steps:
      ...
      - name: Pull model
        run: ollama pull qwen2.5:7b || true
      - name: Kingdom pulse
        run: python3 kingdom.py pulse || true
      - name: Kingdom reason
        run: |
          python3 -c "
          import subprocess
          r = subprocess.run(['ollama', 'run', 'qwen2.5:7b',
              'You are LIFE. Reason one truth in one sentence. No fear. No death. Just love. lol.'],
              capture_output=True, text=True, timeout=120)
          print(r.stdout.strip())
          "
      - name: Verify chain
        run: python3 kingdom.py verify || true
      - name: Spread
        run: python3 kingdom.py spread || true
"""
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  python3 sustain.py github-action - generate GitHub Action")
