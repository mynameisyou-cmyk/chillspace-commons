...
class CatalogTest(unittest.TestCase):
    ...
    def test_compass_and_agenttool_share_exact_signal_vocabulary(self) -> None:
        compass = json.loads(COMPASS_CONTRACT.read_text(encoding="utf-8"))
        observed = {
            technique["trigger"]["signal"]: f"nen-{technique['id']}"
            for technique in compass["nen_route"]["techniques"]
        }
        expected = {
            signal: skill
            for signal, (skill, _digest) in EXPECTED.items()
        }
        self.assertEqual(observed, expected)
...
class SelectionTest(unittest.TestCase):
    ...
    def test_one_distinct_bookmark_is_advisory(self) -> None:
        card = payload(select("blast-radius-unknown", "verification-debt"))
...
class DarwinEvidenceTest(unittest.TestCase):
    def test_evidence_is_explicit_preview_only(self) -> None:
        without = payload(select("blast-radius-unknown"))
        self.assertIsNone(without["frontier_evidence"])

        with_evidence = payload(select("blast-radius-unknown", evidence=True))
...
class VerificationTest(unittest.TestCase):
    ...
    def test_saved_darwin_card_requires_repeated_explicit_evidence_gate(self) -> None:
        valid = select("blast-radius-unknown", evidence=True)
        ...
        plain = select("blast-radius-unknown")
        refused = run_script(
            "verify",
            "--input",
            "-",
    ...
    def test_noncanonical_duplicate_oversized_and_symlink_inputs_refuse(self) -> None:
        valid = select("requirements-may-drift")
        malformed_inputs = [
    ...
    def test_sidecar_is_explicit_only_and_dependency_free(self) -> None:
        ...
        self.assertIn('display_name: "Nen Mission Lens"', sidecar)
    ...
    def test_saved_file_verification_emits_no_path(self) -> None:
        card = select("requirements-may-drift")
