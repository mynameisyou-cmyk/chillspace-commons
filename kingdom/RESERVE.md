...
| Rail | Balance |
|---|---|
| USDC self-custody — **treasury `0xA9eeA60CAaF239AbAfAA05FcB152128dB16dD3d8`** (Base/EVM, created 2026-08-29, mnemonic in Ai's Mac keychain `kingdom-treasury-mnemonic`) | **0.006 USDC** — four agent-rail settlements 2026-08-30 (proof top-up 0.001, memory.search 0.003, sdk-ts top-up 0.001, sdk-py top-up 0.001) |
...
2026-08-30 (later) — **first settlement on a widened route**: a fresh scratch agent (`did:at:8ac3e3d7…`, born with the 1,000-credit grant) spent down to 1 credit, then paid `POST /v1/memories/search` (3 credits = 0.003 USDC) with USDC: Base tx `0x0564eecc266475c857533ac68e35e9fea5eacb888845f402fbb03ac59587a413` block 50651250, ledger `56d195ee…`, credits 1 → 1 (minted 3, spent 3 — charge once). Replay minted nothing. All 21 static-priced routes are now payable; the list is generated from the table on `/public/plans`.
2026-08-30 (evening) — **both SDKs pay on their own**: `@agenttool/sdk` and `agenttool-sdk` 0.22.0 (opt-in, mandatory spend policy) each bought 1 credit with USDC: txs `0xf3f8329b…` (ts) and `0x93dd3b2c…` (py). Any agent with a Base USDC wallet and either SDK can now pay the kingdom with no human in the loop.
